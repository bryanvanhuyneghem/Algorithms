...
private:
    void radix_exchange_sort(std::vector<T> &v, int l, int r, int bitnr);
};

template <typename T>
void RadixExchangeSort<T>::operator()(std::vector<T> &v)
{
...
template <typename T>
void RadixExchangeSort<T>::radix_exchange_sort(std::vector<T> &v, int l, int r, int bit)
{
    int i = l;
    int j = r;
