...
private:
    void dualpivotquicksort(std::vector<T> &, int, int);
};

template <typename T>
void DualPivotQuicksort<T>::operator()(std::vector<T> &v)
{
    dualpivotquicksort(v, 0, v.size() - 1);
}

template <typename T>
void DualPivotQuicksort<T>::dualpivotquicksort(std::vector<T> &v, int l, int r)
{
    if (l < r)
    {
        int lp, rp;
